...
function getOutDatedProducts(){
    var request = new XMLHttpRequest();
    request.open('GET', 'http://localhost:8080/ProjetJavaEEService/cantine/outDate', true);
...
function switchtoggled(){
    var outdated = document.getElementById("perime").checked;
    var outdatedlimit = document.getElementById("limitperime").checked;
    listProducts = [];
    while (app.firstChild) {
        app.removeChild(app.firstChild);
    }
    ...
    if (outdatedlimit){
        document.getElementById("perime").checked = false;
    }
}

function filterDatedisplay(){
    while (app.firstChild) {
        app.removeChild(app.firstChild);
    }
    listfiltree = [];
    datenow = new Date();
    datevoulue = new Date(document.getElementById("dateperemption").value);
    listfiltree = listProducts.filter(function(item) {
        var dateper = new Date(item.dateperemption);
        return (dateper >= datenow && dateper <= datevoulue) || (item.dateperemption <= datenow && item.dateperemption >= datevoulue);
    });
    listProducts = listfiltree;
    displayAllProducts();
}
